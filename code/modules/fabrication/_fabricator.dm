...
/obj/machinery/fabricator
	name = "autolathe"
	desc = "It produces items using metal, glass, plastic, and steel. It has a built in shredder that can recycle most items, although any materials it cannot use will be wasted."
...
/obj/machinery/fabricator/attackby(var/obj/item/O, var/mob/user)
	...
	if(!found_useful_mat)
		to_chat(user, span("warning", "\The [eating] does not contain any accessible useful materials and cannot be accepted."))
		return
	...
	for(var/material in taking_matter)
		...
		amount_used = max(-round(-(amount_available * total_material/taking_matter[material])), amount_used) // Use only as many sheets as needed, rounding up

	if(!amount_used)
		to_chat(user, span("warning", "\The [src] is full. Please remove material from \the [src] in order to insert more."))
		return
	else if(!space_left)
...
/obj/machinery/fabricator/proc/is_functioning()
	. = use_power != 0 && !(stat & NOPOWER) && !(stat & BROKEN) && !(fab_status_flags & FAB_DISABLED)

/obj/machinery/fabricator/Process(var/wait)
	..()
	if(use_power == 2 && (fab_status_flags & FAB_BUSY))
