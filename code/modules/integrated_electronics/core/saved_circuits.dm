...
/proc/save_electronic_assembly(obj/item/device/electronic_assembly/assembly)
	// No components? Don't even try to save it.
	if(!LAZYLEN(assembly.assembly_components))
		return
...
/proc/load_electronic_assembly(loc, list/blocks)

	// Block 1. Assembly.
	var/list/assembly_params = blocks["assembly"]
	var/assembly_path = text2path(assembly_params["path"])
	...
	for(var/component_params in blocks["components"])
		var/component_path = text2path(component_params["path"])
		var/obj/item/integrated_circuit/component = new component_path(assembly)
		assembly.force_add_circuit(component)
