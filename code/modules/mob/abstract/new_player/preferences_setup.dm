...
/datum/preferences/proc/get_jobs_by_dept(var/department_flag)
	switch(department_flag)
		if(CIVILIAN)
			return list(job_civilian_high, job_civilian_med, job_civilian_low)
		if(MEDSCI)
			return list(job_medsci_high, job_medsci_med, job_medsci_low)
		if(ENGSEC)
			return list(job_engsec_high, job_engsec_med, job_engsec_low)
	return null