...
/mob/living/carbon/human/proc/coil_up()
	...
	if(stat || paralysis || !tail_trail)
		return
	visible_message(SPAN_NOTICE("\The [src] coils up!"))
	tail_trail.update_moved(src)
	sleep(-1)
	if(isturf(loc))
		tail_trail.update_moved(loc)
...
/obj/effect/tail/Crossed(var/atom/movable/crossing)
	...
	if(istype(crossing, /mob/living))
		...
		if(M.pulledby == owner || (owner in M.grabbed_by))
			return
		var/damage_taken
		...
		if(prob(50))
			M.visible_message(SPAN_DANGER("\The [M] trips over \the [owner]'s tail!"))
			M.Weaken(rand(5, 8))
		else
			M.visible_message(SPAN_DANGER("\The [M] treads on \the [owner]'s tail!"))
		to_chat(owner, SPAN_DANGER("Your tail is being brutalized!"))
		owner.apply_damage(damage_taken, BRUTE, BP_GROIN)
	else if(istype(crossing, /obj/vehicle) || ismech(crossing))
		crossing.visible_message(SPAN_DANGER("\The [crossing] runs over \the [owner]'s tail!"))
		to_chat(owner, SPAN_DANGER("Your tail is being brutalized!"))
...
/obj/effect/tail/update_icon(var/force_override)
	...
	add_overlay(human_tail_cache[cache_key])
	// Build markings.
...
/obj/effect/tail/trailing/update_icon()
	if(moved_from != 0 && reverse_dir[moved_from] == dir)
