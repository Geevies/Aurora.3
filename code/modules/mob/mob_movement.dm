...
/client/Move(new_loc, direct)
	...
	if(isliving(mob))
		...
		if(L.incorporeal_move && isturf(mob.loc))//Move though walls
			...
			Process_Incorpmove(direct, mob)
			return
		if(mob.client && ((mob.client.view != mob.client.get_hud_view()) || (mob.client.pixel_x != mob.client.hud_pixel_x) || (mob.client.pixel_y != 0)))		// If mob moves while zoomed in with device, unzoom them.
