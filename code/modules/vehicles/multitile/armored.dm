...
var/global/list/armorvic_dmg_distributions = 
	list(HDPT_PRIMARY = 0.15,
		HDPT_SECDGUN = 0.125,
		HDPT_SUPPORT = 0.075,
		HDPT_ARMOR = 0.5,
		HDPT_TREADS = 0.15
...
/obj/vehicle/multitile/root/armored
	name = "Armored Vehicle"
	desc = "Get inside to operate the vehicle."
	hitbox_type = /obj/vehicle/multitile/hitbox/armored //Used for emergencies and respawning hitboxes
...
/obj/vehicle/multitile/root/armored/Initialize()
	. = ..()
	GLOB.tank_list += src
	set_light(15)


/obj/vehicle/multitile/root/armored/Destroy()
...
/obj/vehicle/multitile/root/armored/proc/handle_all_modules_broken()
	return

/obj/vehicle/multitile/root/armored/proc/deactivate_all_hardpoints()
	var/list/slots = get_activatable_hardpoints()
	for(var/slot in slots)
		var/obj/item/hardpoint/HP = hardpoints[slot]
		HP?.deactivate()

/obj/vehicle/multitile/root/armored/proc/remove_all_players()
	return

//The basic vehicle code that moves the tank, with movement delay implemented
/obj/vehicle/multitile/root/armored/relaymove(mob/user, direction)
...
/obj/vehicle/multitile/root/armored/try_rotate(deg, mob/user, force = FALSE)
	if(world.time < next_move && !force)
		return
	next_move = world.time + move_delay * misc_ratios["move"] * (force ? 2 : 3) //3 for a 3 point turn, idk
	return ..()

/obj/vehicle/multitile/root/armored/proc/can_use_hp(mob/M)
	return TRUE
...
/obj/vehicle/multitile/root/armored/verb/switch_active_hp()
...
/obj/vehicle/multitile/root/armored/verb/reload_hp()
...
/obj/vehicle/multitile/root/armored/proc/finish_reloading_hp(mob/living/user, obj/item/hardpoint/HP, obj/item/ammo_magazine/A, slot)
	if(!can_use_hp(usr))
		return
...
/obj/vehicle/multitile/root/armored/proc/get_activatable_hardpoints()
	var/list/slots = list()
...
/obj/vehicle/multitile/root/armored/load_hitboxes(datum/coords/dimensions, datum/coords/root_pos)
	...
	for(var/i = start_x to end_x)
		...
		for(var/j = start_y to end_y)
			...
			var/obj/vehicle/multitile/hitbox/armored/H = new(locate(x + C.x_pos, y + C.y_pos, z))
			H.setDir(dir)
			H.root = src
			linked_objs[C] = H

/obj/vehicle/multitile/root/armored/load_entrance_marker(datum/coords/rel_pos)
...
/obj/vehicle/multitile/root/armored/examine(mob/user)
	..()
...
/obj/vehicle/multitile/root/armored/proc/healthcheck()
...
/obj/vehicle/multitile/root/armored/update_icon()
...
/obj/vehicle/multitile/hitbox/armored
	name = "Armored Vehicle"
	desc = "Get inside to operate the vehicle."
...
/obj/vehicle/multitile/hitbox/armored/Destroy()
	var/obj/vehicle/multitile/root/armored/C = root
	C?.take_damage_type(1000000, "abstract")
	return ..()

//Tramplin' time, but other than that identical
/obj/vehicle/multitile/hitbox/armored/Bump(atom/A)
	. = ..()
...
/obj/vehicle/multitile/hitbox/armored/proc/get_driver()
	return "Someone"

/atom/proc/tank_collision(obj/vehicle/multitile/hitbox/armored/C, facing, turf/T, turf/temp)
	return

/mob/living/tank_collision(obj/vehicle/multitile/hitbox/armored/C, facing, turf/T, turf/temp)
	...
	var/obj/vehicle/multitile/root/armored/CA = C.root
	var/list/slots = CA.get_activatable_hardpoints()
	for(var/slot in slots)
		var/obj/item/hardpoint/H = CA.hardpoints[slot]
		H?.livingmob_interact(src)

/mob/living/carbon/xenomorph/queen/tank_collision(obj/vehicle/multitile/hitbox/armored/C, facing, turf/T, turf/temp)
...
/mob/living/carbon/xenomorph/crusher/tank_collision(obj/vehicle/multitile/hitbox/armored/C, facing, turf/T, turf/temp)
	if(lying_angle || loc == C.loc)
		return ..()
	temp = get_step(T, facing)
	T = temp
...
/mob/living/carbon/xenomorph/larva/tank_collision(obj/vehicle/multitile/hitbox/armored/C, facing, turf/T, turf/temp)
	if(loc == C.loc) // treaded over.
		ParalyzeNoChain(20)
		apply_damage(rand(5, 7.5), BRUTE)
		return
...
/turf/closed/wall/tank_collision(obj/vehicle/multitile/hitbox/armored/C, facing, turf/T, turf/temp)
	var/obj/vehicle/multitile/root/armored/tank/CA = C.root
	var/damage = 30
	var/tank_damage = 2
...
/obj/machinery/tank_collision(obj/vehicle/multitile/hitbox/armored/C, facing, turf/T, turf/temp)
	var/obj/vehicle/multitile/root/armored/tank/CA = C.root
	var/damage = 30
	var/tank_damage = 2
...
/obj/structure/tank_collision(obj/vehicle/multitile/hitbox/armored/C, facing, turf/T, turf/temp)
	var/obj/vehicle/multitile/root/armored/tank/CA = C.root
	var/damage = 30
	var/tank_damage = 2
...
/obj/effect/alien/tank_collision(obj/vehicle/multitile/hitbox/armored/C, facing, turf/T, turf/temp)
	take_damage(40)

/obj/effect/alien/weeds/tank_collision(obj/vehicle/multitile/hitbox/armored/C, facing, turf/T, turf/temp)
	return

/obj/vehicle/multitile/hitbox/armored/Move(atom/A, direction)
...
/obj/vehicle/multitile/hitbox/armored/projectile_hit(obj/projectile/proj)
...
/obj/vehicle/multitile/hitbox/armored/bullet_act(obj/projectile/P)
	return root.bullet_act(P)

/obj/vehicle/multitile/hitbox/armored/ex_act(severity)
	return root.ex_act(severity)

/obj/vehicle/multitile/hitbox/armored/attackby(obj/item/I, mob/user, params)
	return root.attackby(I, user, params)

/obj/vehicle/multitile/hitbox/armored/attack_alien(mob/living/carbon/xenomorph/X, damage_amount = X.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = "", effects = TRUE, armor_penetration = 0, isrightclick = FALSE)
	return root.attack_alien(X, damage_amount)

/obj/vehicle/multitile/hitbox/armored/effect_smoke(obj/effect/particle_effect/smoke/S)
	. = ..()
	if(!.)
		return
	if(CHECK_BITFIELD(S.smoke_traits, SMOKE_XENO_ACID))
		var/obj/vehicle/multitile/root/armored/T = root
...
/obj/vehicle/multitile/root/armored/Adjacent(atom/A)
	for(var/i in linked_objs)
		var/obj/vehicle/multitile/hitbox/armored/H = linked_objs[i]
		if(!H)
			continue
		if(get_dist(H, A) <= 1)
...
/obj/vehicle/multitile/root/armored/proc/get_dmg_multi(type)
...
/obj/vehicle/multitile/root/armored/proc/take_damage_type(damage, type, atom/attacker)
	for(var/i in hardpoints)
		var/obj/item/hardpoint/HP = hardpoints[i]
		if(HP)
...
/obj/vehicle/multitile/root/armored/projectile_hit(obj/projectile/proj)
...
/obj/vehicle/multitile/root/armored/ex_act(severity)
...
/obj/vehicle/multitile/root/armored/attack_alien(mob/living/carbon/xenomorph/M, damage_amount = M.xeno_caste.melee_damage, damage_type = BRUTE, damage_flag = "", effects = TRUE, armor_penetration = 0, isrightclick = FALSE)
...
/obj/vehicle/multitile/root/armored/attack_hand(mob/living/user)
	. = ..()
	if(.)
		return
...
/obj/vehicle/multitile/root/armored/Entered(atom/movable/A)
...
/obj/vehicle/multitile/root/armored/proc/update_damage_distribs()
...
/obj/vehicle/multitile/root/armored/attackby(obj/item/O, mob/user)
...
/obj/vehicle/multitile/root/armored/proc/handle_hardpoint_repair(obj/item/O, mob/user)
...
/obj/vehicle/multitile/root/armored/proc/handle_ammomag_attackby(obj/item/ammo_magazine/AM, mob/user)
...
/obj/vehicle/multitile/root/armored/proc/install_hardpoint(obj/item/hardpoint/HP, mob/user)
...
/obj/vehicle/multitile/root/armored/proc/uninstall_hardpoint(obj/item/O, mob/user)
...
/obj/vehicle/multitile/root/armored/proc/add_hardpoint(obj/item/hardpoint/HP, mob/user)
	if(!istype(HP))
		return
	HP.owner = src
	if(HP.obj_integrity)
...
/obj/vehicle/multitile/root/armored/proc/remove_hardpoint(obj/item/hardpoint/old, mob/user)
...
/obj/vehicle/multitile/root/armored/contents_explosion(severity)
	return